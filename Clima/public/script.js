...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function buscarClima() {
        const cidade = campoCidade.value.trim();  // Correção aqui
        if (!cidade) {
        ...
        try {
            ...
            vento.textContent = Math.round(dados.wind.speed * 3.6);  // Convertendo de m/s para km/h
            descricao.textContent = dados.weather[0].description;
        } catch (erro) {
            alert("Não foi possível buscar essa cidade");  // Correção na palavra "possível"
        }
    }
});
